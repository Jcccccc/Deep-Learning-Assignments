...
class flatten(object):
    ...
    def forward(self, feat):
        ...
        for i in range(feat.shape[0]):
            out_list.append(feat[i,:].flatten())
    ...
    def backward(self, dprev):
        ...
        dfeat_list = list()
        for i in range(feat.shape[0]):
            dfeat_list.append(np.reshape(dprev[i,:], feat.shape[1:]))
        dfeat = np.array(dfeat_list)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class relu(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        dfeat = np.array((self.meta>0) * 1) * dprev
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if self.keep_prob == 0:
            self.keep_prob = 1
        if is_training:
            kept = (self.rng.uniform(0.0, 1.0, feat.shape) <= self.keep_prob).astype(int)
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        label_vector = np.zeros(feat.shape)
        label_vector[range(feat.shape[0]), label] = 1
        label = label_vector
    ...
    def backward(self):
        ...
        #############################################################################
        dlogit = - (self.label - self.logit) / self.label.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = None
